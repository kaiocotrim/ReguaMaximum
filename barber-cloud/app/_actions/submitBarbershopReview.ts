...
export async function submitBarbershopReview(input: {
  bookingId: string
  rating: number
  comment: string
}) {
  ...
  if (!booking) {
    return {
      success: false as const,
      error: "Avalie o barbeiro primeiro ou esta avaliação já foi enviada.",
    }
  }
  ...
  try {
    ...
    return { success: false as const, error: "Esta avaliação já foi enviada." }
  }
